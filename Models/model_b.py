...
Original file is located at
    https://colab.research.google.com/drive/1PODG_WATuCdKwZWgfLbp8t_T_G1s00FX
"""
...
X2 = np.load("SecondPSSM.npy")
# X3 = np.load("bpf.npy")
X4 = np.load("Secondary_physico_31.npy")
...
for train_index,test_index in kf.split(X2,Y):
    ...
    result.append(model.fit(x=[X2[train_index,:,:],X4[train_index,:,:]],y=Y[train_index],validation_data=([X2[test_index,:,:],X4[test_index,:,0:]],Y[test_index]), epochs=epoch_value, batch_size=15,callbacks=callbacks_list))
    ...
    scores = model.evaluate([X2[test_index,:,:],X4[test_index,:,:]], Y[test_index], verbose=0)
    ...
    probabilities = model.predict([X2[test_index,:,:],X4[test_index,:,:]])
    ...
    acc, precision, sensitivity, specificity, MCC, F1 = calculate_performace(len(test_index), predicted_classes, Y[test_index])
